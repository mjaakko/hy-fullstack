...
const CreateNew = withRouter((props) => {
  ...
  const handleSubmit = (e) => {
    props.displayNotification(`Created new anecdote '${content}'`)
    e.preventDefault()
    props.addNew({
      content,
      author,
...
const App = () => {
  ...
  return (
    <Router>
      <div>
        { notification && <div style={{ margin: 10, borderStyle: 'solid', borderWidth: 2, borderColor: 'black'}}>{ notification }</div> }
        ...
        <Route path="/create" render={() => <CreateNew addNew={addNew} displayNotification={ displayNotification }/>}></Route>
        <Footer />
      </div>
    </Router>
  )
